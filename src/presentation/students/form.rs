...
use super::{clear_form, StudentsState};

pub fn show(ctx: &egui::Context, repo: &Arc<dyn StudentRepo>, state: &mut StudentsState, notifs: &mut Notifications) {
    if !state.show_modal { return; }

    let is_edit = state.editing_id.is_some();
    let title   = if is_edit { "Editar Alumno" } else { "Nuevo Alumno" };
    ...
    egui::Window::new(title)
        ...
        .show(ctx, |ui| {
            ui.with_layout(egui::Layout::top_down_justified(egui::Align::LEFT), |ui| {
                ...
                ui.label(egui::RichText::new("Tipo").color(colors::LIGHT_GRAY).size(sizes::FONT_SIZE_NORMAL));
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui.button("Guardar").clicked() {
                            ...
                            let result = if is_edit {
                                StudentUpdateUseCase::new(Arc::clone(repo)).execute(StudentUpdateInput {
                                ...
                                StudentCreateUseCase::new(Arc::clone(repo)).execute(StudentCreateInput {
                                    age_group:  state.age_group,
                                    email:      state.email.clone(),
                                    first_name: state.first_name.clone(),
                                    last_name:  state.last_name.clone(),
