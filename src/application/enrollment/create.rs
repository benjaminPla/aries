...
impl EnrollmentCreateUseCase {
    ...
    pub fn execute(&self, input: EnrollmentCreateInput) -> Result<(), EnrollmentAppError> {
        let course  = self.course_repo.get_by_id(input.course_id)
            .map_err(|_| EnrollmentAppError::NotFound)?;
        let student = self.student_repo.get_by_id(input.student_id)
            .map_err(|_| EnrollmentAppError::NotFound)?;
        ...
        let active = self.enrollment_repo.count_active(input.course_id)
            .map_err(|e| EnrollmentAppError::Database(e.to_string()))?;
